...
import com.xujl.etl.xpath.extractor.Extractor.Callback;
import com.xujl.etl.xpath.extractor.impl.XMLExtractor;
import com.xujl.etl.xpath.schema.*;
import com.xujl.etl.xpath.schema.Config.Pages;
import com.xujl.etl.xpath.schema.Config.Seeds;
                ...
                switch (entry.getModel().getName()) {
                        ...
                        if (K.isNotBlank(bindingsClassName)) {
                            ...
                            if (bindingsClass != null) {
                                ...
                                try {
                                    ...
                                    throw new RuntimeException("实例化" + bindingsClassName + "失败", e);
                                }
                    ...
                    case "filter":
                        ...
                        try {
                            ...
                            throw new RuntimeException("过滤器[class=" + ftCls.getName() + "]实例化失败", e);
                        }
                    ...
                    case "extract-page":
                        ...
                        if (K.isNotBlank(filterName)) {
                            ...
                            if (ft == null) {
                                throw new RuntimeException("页面[name=" + pageName + "]指定的的filter[name=" + filterName + "]不存在");
                            }
                        ...
                        if (rule == null) {
                            throw new RuntimeException("页面[name=" + pageName + "]缺少URL匹配规则的配置");
                        }
                            ...
                            models.forEach(mdl -> {
                                ...
                                if (K.isNotEmpty(_fields)) {
                                    _fields.forEach(f -> {
                                        ...
                                        final  String parent = f.getString("parent");
                                        final Field parentField = model.getField(parent);
                                        final Field field = parentField == null? model.addField(fieldName):parentField.addField(fieldName);
                                        field.putAll(f);
                                        // 处理Filters
                                        ...
                                        if (K.isNotBlank(ftName)) {
                                            ...
                                            if (rft == null) {
                                                throw new RuntimeException("页面[name=" + pageName + "].模型[name=" + modelName + "].Field[name=" + fieldName + "]配置的属性[filter=" + ftName + "]不存在");
                                            }
                                            field.addFilter(rft);
                                        }
...
//                                                        field.addFilter(new ScriptableFilter(ft.getString("value", ft.getString("text"))));
    ...
    private void handleExtractor(Page page, String extractorName) {
        ...
        if (K.isBlank(extractorName)) {
            throw new RuntimeException("页面[name=" + pageName + "]必须指定解析器");
        }
        ...
        if (extractorClass == null) {
            throw new RuntimeException("页面[name=" + pageName + "]指定的extractor[name=" + extractorName + "]不存在");
        }
        final Constructor<? extends Extractor> ct;
        try {
            ct = extractorClass.getConstructor( String.class, Model[].class);
        } catch (Exception e) {
            throw new RuntimeException("页面[name=" + pageName + "]指定的extractor[class=" + extractorClass.getName() + "]的构造器不满足要求", e);
        }
        if (ct == null) {
            throw new RuntimeException("页面[name=" + pageName + "]指定的extractor[class=" + extractorClass.getName() + "]的构造器不满足要求");
        }
        page.setExtractorBuilder(( p, mds) -> {
            try {
                return ct.newInstance(p, mds);
            } catch (Exception e) {
                throw new RuntimeException("页面[name=" + pageName + "]指定的extractor[class=" + extractorClass.getName() + "]实例化失败", e);
            }
        });
    }
